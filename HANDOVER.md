...
最終更新: 2026-05-30
最新コミット: `cd1e5b6 シート交換: 画面見出しを「マシンシート交換依頼」に変更`（develop ブランチ push 済み）

> **2026-05-30 検証者共有時点の状態**: 開発DBを「本番相当マスタ」に洗い替え済み（ゾーン4 / エリア18 / 店舗143 / カテゴリ fitness・golf / 仕入先9 / 商品176（全件に仮画像3枚）/ ユーザー165）。発注一覧の動作確認用に約3,000件のテスト発注を投入済み。短縮コードは店舗コード5桁を流用（採番は `REP-30101-...`）。詳細は §4-4 / §7-23〜。

---

## 1. プロジェクト概要

店舗向け発注管理システム（修理・備品・部品・シート交換の4種別 + 予算管理 + 自店調達申請 + マスタメンテ + 監査ログ）。
...
4. 既存DBがある場合は、必要な migration を順次適用:
   ...
   ```
   ※ `schema.sql` には上記すべて統合済み（short_code は VARCHAR(5)）。新規構築時は schema.sql 一本で OK。

### 4-4b. 本番相当マスタ＋検証データの投入（2026-05-30〜）

開発/検証環境は、上記の `seed.sql`（旧サンプル30店舗）ではなく **本番相当の実マスタ** を使用します。

**■ 検証者へ渡す場合（推奨）: DBダンプを共有**
- 現環境を `mysqldump -u root kaikatsu > kaikatsu_dump.sql` で書き出し、検証者はそれを取り込むのが最も簡単・確実（マスタ＋仮画像パス＋テスト発注まで揃った状態）。
...
4. 発注一覧の検証用に約3,000件のテスト発注を投入（2店舗×全種別×全ステータス×2の構造化160件＋充填。詳細/履歴/予算反映つき）:
   ...
   ```
- パスワードは seed_master_real.sql 内で bcrypt（=`password`）を直接投入済みのため、本マスタ利用時は `hash_passwords.php` 不要。
...
本番相当マスタ（2026-05-30〜）でのログインアカウント（**初期パスワードは全員 `password`**）:

| login_id | password | role | 備考 |
|---|---|---|---|
| admin | password | admin | 商品部 管理者 |
...
### 7-24. 本番相当マスタへの洗い替え＋検証データ整備（2026-05-30）
快活提供の収集シートを基に、開発DBのマスタを実データへ洗い替え。
- ゾーン4 / エリア18 / 店舗143 / カテゴリ fitness・golf / 仕入先9 / 商品176 / ユーザー165（admin1・shop143・zone4・area17）/ 予算（仮値）
- 変換: `docs/build_master_seed.py` → `database/seed_master_real.sql`（**個人情報含むため gitignore**）
- **短縮コード**: `shops.short_code` に**店舗コード5桁を流用**（列を VARCHAR(3)→(5) に拡張）。発注番号は `REP-30101-...` に
- 商品コードはカテゴリ別連番（FIT-/GLF-）で自動採番。**保留8件**（複数コード/複数金額/単価不備）は未登録
- 全商品に**仮画像3枚**（`tools/gen_product_placeholders.php`、16:9 640×360、uploads/products）
- 発注一覧の検証用に**約3,000件のテスト発注**（`tools/seed_orders_volume.php`）
- 既知の保留事項: ZM→ゾーンは仮割当 / KF中部エリアにAM不在 / 予算は仮値 / 保留商品8件 / golf商品の写真も仮

### 7-25. 各種UX改善・表記統一（2026-05-30）
- **発注一覧**: 初期表示の発注日Fromを**直近3ヶ月**に既定設定（件数増時の初期取得抑制。過去分はFrom空で表示）
- **予算管理**: 店舗ドロップダウンを**選択カテゴリで絞り込み**（フィットネス選択時にゴルフ専用店を除外。shops API が categories を返却）／折りたたみ月別明細を**遅延描画**（初期描画の軽量化）
- **共通ローディング**: オーバーレイを**遅延表示(250ms)**にして高速処理時のチラつき防止（`common-nav.js`）
- **備品発注**: 予算超過を**カートバー（常時表示）にも反映**（⚠予算超過＋合計赤）／数量±時に**該当カードのみ更新**して画像のちらつき解消／タップ青ハイライト・誤選択を抑制／商品名検索の**IME変換中は検索しない**
- **発注メール下書き**: 店舗名の重複「店」修正＋**仕入先商品コード**表示
- **表記統一**: 全画面の見出し/タブから「画面」を除去（管理メニュー等と平仄）／シート交換の見出しを「マシンシート交換依頼」に／写真添付文言を「タップして写真を選択」に
...
### 9-1. 直近の未完了タスク
- [~] **業務フロー設計書 PPTX の最新化** — 元図 `docs/bizflow-slides/*.svg` / 生成 `docs/generate_bizflow_pptx.py` → 出力 `docs/bizflow_design_v3.pptx`（2026-05-30 生成）。
  - **更新済み（v3）**: 09_ステータス遷移（自動遷移バッチ廃止＝全工程手動、シート交換を補足に反映）／11_予算サイクル（**納品月ベース計上＋発注時の仮計上アラート**のタイムライン構成に刷新）。
  - **未反映**: 06〜08 の各フロー図（修理/備品/部品）にシート交換フロー図を追加する等は別途。各スライドの「図形に変換」（PowerPoint手動／`--convert-shapes`）と表紙作成も手動工程として残る。
  - 検証者向けには**技術ドキュメント（md / DB定義書 / 開発状況xlsx）＋ v3 PPTX（予算・ステータス最新）**を共有。`bizflow_design_v3.pptx` はSVGネイティブ埋め込み版（PowerPointで「図形に変換」可能）。

### 9-2. 中期で残っている開発項目

詳細は **[docs/快活システム_画面機能一覧_開発状況.xlsx](docs/快活システム_画面機能一覧_開発状況.xlsx)** を参照（2026-05-30 時点）。
...
#### 主な残作業（2026-05-30 時点）
- 本番デプロイ（さくらレンタルサーバー）準備（cron 登録 = apply_scheduled_changes のみ）
- 本番 SMTP 設定（Mailpit → 本番メールサーバへ切替）。※実アドレス送信になるため切替時は要注意（Mailpit中は実送信されない）
- マスタ収集シートの**保留商品8件**の確定登録（複数コード/複数金額/単価不備）
- ZM→ゾーンの正式割当（現在は仮）/ KF中部エリアのAM補完 / 予算の確定値投入（現在は仮値）
- 業務フロー設計書PPTXの最新化（§9-1）
